...
function browserSyncEnvStatus() {
  return {
    ...
    note: '0.2.39 keeps V31/V34 POS browser sync untouched and adds Uber/Frieda theoretical fallback display when online/uploaded external sales are missing.'
  };
}
...
app.get('/api/diagnostics/browser-sync', asyncRoute(async (_req, res) => {
  res.json({ ok: true, version: '0.2.39', diagnostics: await browserSyncProbe() });
}));
...
app.get('/api/live/summary', asyncRoute(async (req, res) => {
  ...
  // 0.2.39: if the new Render service has no import cache yet, lazily load
...
app.post('/api/sync/pos/day', asyncRoute(async (req, res) => {
  // 0.2.39: when browser sync is enabled, the normal POS button uses the same
...
app.post('/api/captures/browser', (req, res) => {
  ...
  saveLive(next);
  res.json({ ok: true, parsed, live: liveMerged(), note: 'Manual capture is emergency/diagnostic only. Normal 0.2.39 workflow is file-import baseline plus isolated POS day/backfill fallback.' });
});
...
app.listen(PORT, () => console.log(`Freda Ops Cockpit Beta 0.2.39 running on http://localhost:${PORT}`));
...
async function runPosBrowserSequential(req, res) {
  ...
  for (const store of stores) {
    ...
    runs.push(run);
    next = applySyncResult(next, run);
    // 0.2.39: save immediately after each store, so a later store timeout cannot
...
async function runPosBenchmarks(req, res) {
  ...
  // 0.2.39: this is the Freda-facing "Sync current + last days" path.
