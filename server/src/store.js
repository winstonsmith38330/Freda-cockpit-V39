...
export function applySyncResult(state, result) {
  let next = { ...state, version: '0.2.39', reportingDate: result.reportingDate || state.reportingDate, updatedAt: new Date().toISOString() };
...
function buildLiveAwareWeeklySummary(live = {}) {
  ...
  const summary = {
    ...
    note: '0.2.39 source policy: POS reporting.site live sync remains primary. Uber/Square use online actual first, uploaded backup second, and theoretical fallback from last-4 same weekday plus last-4 WTD pattern when actual data is missing.'
  };
  return summary;
}
